...
!function(e){jQuery().draggable||(e.fn.draggable=function(){return this.css("cursor","move").on("mousedown touchstart",function(a){{var t=e(this),o=t.offset().left-a.pageX,n=t.offset().top-a.pageY;t.css("z-index")}e.fn.draggable.stack||(e.fn.draggable.stack=999),stack=e.fn.draggable.stack,e(window).on("mousemove.draggable touchmove.draggable",function(e){t.css({"z-index":stack,transform:"scale(1.1)",transition:"transform .3s",bottom:"auto",right:"auto"}).offset({left:o+e.pageX,top:n+e.pageY}).find("a").one("click.draggable",function(e){e.preventDefault()}),e.preventDefault()}).one("mouseup touchend touchcancel",function(){e(this).off("mousemove.draggable touchmove.draggable click.draggable"),t.css({"z-index":stack,transform:"scale(1)"}),e.fn.draggable.stack++}),a.preventDefault()}),this})}(jQuery);

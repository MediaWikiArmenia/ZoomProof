...
ZoomProof.prototype.prepareData = function() {
...
ZoomProof.prototype.update = function() {
	b = this.editor.value;
	var diff = JsDiff.diffWordsWithSpace(a, b);
	mp = [];
	
	var ap = 0;
	var bp = 0;
	
	mp = diff.map(function(diff) {
...
ZoomProof.prototype.find = function(position) {
	...
	for(var i = 0; i < this.data.words.length; ++i) {
		...
		if(position >= p && position <= p + word.contents.length) {
			return word;
		}
...
ZoomProof.prototype.enable = function(json) {
	...
	var $wikiEditorUI = $(".wikiEditor-ui");
	...
	$zoomProofUI.appendTo($wikiEditorUI);
	
	
	$editor.on('keyup click focus', function() {
		self.update(); // Needs optimizations. TODO
		
		var selectionStart = $editor.prop("selectionStart");
		var selectionEnd  = $editor.prop("selectionEnd");
		
		if(selectionStart !== selectionEnd) { // TODO
			console.log('Selection is range!');
			return;
		}
		
		var i;
		for(i = 0; i < mp.length; ++i) {
			if(selectionStart >= mp[i].startB && selectionStart <= mp[i].endB) {
				break;
			}
		}
		if(i == mp.length) { // TODO
			console.log('Corresponding point was not found!');
			return;
		}
		...
		var j = i;
		while (!isKeyPoint(mp[j])) {
			if(j-- === 0) {
				break;
			}
		}
		if(j === -1) { // TODO
			console.log('Previous keypoint was not found!');
			return;
		}
		
		var k = i;
		while (!isKeyPoint(mp[k])) {
			if(++k === mp.length) {
				break;
			}
		}
		if(k === mp.length) { // TODO
			console.log('Next keypoint was not found!');
			return;
		}
		
		
		var lengthA = mp[k].endA - mp[j].startA;
		console.log(a.substr(mp[j].startA, lengthA));
		
		var lengthB = mp[k].endB - mp[j].startB;
		console.log(b.substr(mp[j].startB, lengthB));
		
		var offset = (selectionStart - mp[j].startB) / lengthB;
		var word = self.find(mp[j].startA + Math.round(offset * lengthA));
